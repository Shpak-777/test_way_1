...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		$this->load->model('tool/image');
				...
				if (isset($xds_coloring_theme['left_subcategory'])) {
					$data['subcategory_left'] = $xds_coloring_theme['left_subcategory'];
				}
				
				if (isset($xds_coloring_theme['disable_cart_button'])) {
					$data['disable_cart_button'] = $xds_coloring_theme['disable_cart_button'];
				}

				if (isset($xds_coloring_theme['disable_cart_button_text'])) {
					$data['disable_cart_button_text'] = $xds_coloring_theme['disable_cart_button_text'][$language_id];
				}
				
				$data['on_off_qview'] = false;
				if (isset($xds_coloring_theme['on_off_qview'])) {
					$data['on_off_qview'] = $xds_coloring_theme['on_off_qview'];
				}

				$data['qview_text'] = $this->language->get('qview_text');
				
				if (isset($xds_coloring_theme['category_description_position'])) {
					$data['description_position'] = $xds_coloring_theme['category_description_position'];
				}
        ...
        if ($this->currency->has($this->config->get('config_currency2')) && ($this->config->get('config_currency2') != $this->session->data['currency'])) {
            $special .= ' (' . $this->currency->format($this->tax->calculate($result['special'], $result['tax_class_id'], $this->config->get('config_tax')), $this->config->get('config_currency2')) . ')';
				...
				$for_oprions = $this->model_catalog_product->getProductOptions($result['product_id']);
				$end_options = array();
				foreach ($for_oprions as $option) {
					$product_option_value_data = array();

					foreach ($option['product_option_value'] as $option_value) {
						if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
							if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && (float)$option_value['price']) {
								$pricett = $this->currency->format($this->tax->calculate($option_value['price'], $result['tax_class_id'], $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']);
							} else {
								$pricett = false;
							}
