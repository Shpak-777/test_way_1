  ...
  <div class="row">
    ...
    <div id="content" class="<?php echo $class; ?>">
        ...
        <div class="col-lg-9 col-md-8 col-sm-7">
			...
			<table frame="border" border="1" style="border: 1px solid #000000;width:100%;margin:3px 0;">
				<?php 
				$th_m = array();
				$tr_m = array();
				foreach ($options as $option) { ?>
					<?php if ($option['type'] == 'select') { ?>
						<?php foreach ($option['product_option_value'] as $option_value) { ?>
							<?php if(trim($option_value['name'])=='3 дня') {
								$th_m[0] = trim($option_value['name']);
								$tr_m[0] =  number_format((int)$option_value['price_value'] + (int)((!$special) ? preg_replace("/[^\d.]/","",rtrim($price, " \t.")) : preg_replace("/[^\d.]/","",rtrim($special, " \t."))), 0, '.', '').'р.';
							} ?>
							<?php if(trim($option_value['name'])=='7 дней') {
								$th_m[1] = '1 НЕДЕЛЯ';
								$tr_m[1] = number_format((int)$option_value['price_value'] + (int)((!$special) ? preg_replace("/[^\d.]/","",rtrim($price, " \t.")) : preg_replace("/[^\d.]/","",rtrim($special, " \t."))), 0, '.', '').'р.';
							} ?>
							<?php if(trim($option_value['name'])=='14 дней') {
								$th_m[2] = '2 НЕДЕЛИ';
								$tr_m[2] = number_format((int)$option_value['price_value'] + (int)((!$special) ? preg_replace("/[^\d.]/","",rtrim($price, " \t.")) : preg_replace("/[^\d.]/","",rtrim($special, " \t."))), 0, '.', '').'р.';
							} ?>
						<?php } ?>
					<?php } ?>
				<?php } 
				//print_r(((!$special) ? $price : $special));
				echo '<tr><th style="text-align: center; padding:3% 0;" width="33%">'.implode('</th><th style="text-align: center; padding:3% 0;" width="33%">', $th_m).'</th></tr>';
			 echo '<tr><td style="text-align: center; padding:3% 0;" width="33%">'.implode('</td><td style="text-align: center; padding:3% 0;" width="33%">', $tr_m).'</td></tr>';
				?>
			</table>
			<?php } ?>
							<?php if ($images) { ?>
        ...
        <div class="col-lg-3 col-md-4 col-sm-5" id="product">
          <div class="panel panel-default">
                        ...
                        -->
						<?php if ($price) { 
							$prod_price = preg_replace("/[^0-9]/", '', $price); ?>
						<div class="price" itemprop = "offers" itemscope itemtype = "http://schema.org/Offer">
							...
							$prod_price = preg_replace("/[^0-9]/", '', $special);
          ...
          <div class="options">
            ...
            <div class="form-group">
              ...
              <select name="option[<?php echo $option['product_option_id']; ?>]" id="input-option<?php echo $option['product_option_id']; ?>" class="form-control">
                ...
                <?php foreach ($option['product_option_value'] as $option_value) { ?>
                <option value="<?php echo $option_value['product_option_value_id']; ?>" data-points="<?php echo (isset($option_value['points_value']) ? $option_value['points_value'] : 0); ?>" data-prefix="<?php echo $option_value['price_prefix']; ?>" data-price="<?php echo $option_value['price_value']; ?>"><?php echo $option_value['name'].' (за один день '.number_format((((int)$option_value['price_value'] + (int)$prod_price)/((int)preg_replace("/[^0-9]/", '', $option_value['name']))), 0, '.', '').'р.)'; ?>
                <?php if ($option_value['price']) { ?>
                <!--(<?php echo ($option_value['price_prefix'] == '+' || $option_value['price_prefix'] == '-' ? $option_value['price_prefix'] : '') . (number_format((((int)$option_value['price_value'] + (int)$prod_price)/((int)preg_replace("/[^0-9]/", '', $option_value['name']))), 0, '.', '')); ?>)-->
                <?php } ?>
                </option>
                <?php } ?>
              </select>
              ...
              <div id="input-option<?php echo $option['product_option_id']; ?>">
                ...
                <div class="radio-checbox-options">
                  <input type="radio" name="option[<?php echo $option['product_option_id']; ?>]" value="<?php echo $option_value['product_option_value_id']; ?>" data-points="<?php echo (isset($option_value['points_value']) ? $option_value['points_value'] : 0); ?>" data-prefix="<?php echo $option_value['price_prefix']; ?>" data-price="<?php echo $option_value['price_value']; ?>" id="<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>" />
                    ...
                    <?php if ($option_value['price']) { ?>
                    <span class="option-price"><?php echo ($option_value['price_prefix'] == '+' || $option_value['price_prefix'] == '-' ? $option_value['price_prefix'] : '') . $option_value['price']; ?></span>
                    <?php } ?>
                  </label>
                </div>
                <?php } ?>
            ...
            <?php } ?>
					...
					<div class="row">
						...
						<?php if (($product_quantity <= 0) and $disable_cart_button){ ?>
							<button type="button" id="button-cart" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-block btn-default " disabled>Арендовать<?php // echo $disable_cart_button_text; ?></button>
							<?php } else {  ?>
							<button type="button" id="button-cart" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-block btn-danger ">Арендовать<?php //echo $button_cart; ?></button>
							<?php }  ?>
						</div>
					    
						
            </div>
                        <div class="zakazat-zvonok-produkt" style="width: 100%; margin: 10px auto;">
					        <a class="btn" href="javascript:void(0)" onclick="get_modal_callbacking()" style="width: 100%; color: #fff; background: #337ab7;" id="wishlist-total">Заказать звонок</a>
						</div>
